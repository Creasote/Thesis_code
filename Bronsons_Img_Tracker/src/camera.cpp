...
/*
Capture function added by Bronson Philippa to give a convenient way to acquire an openCV Mat
*/
...
cv::Mat CCamera::Capture(int level)
{
	return Outputs[level]->Capture();
}
...
bool CCameraOutput::Init(int width, int height, MMAL_COMPONENT_T* input_component, int input_port_idx, bool do_argb_conversion)
{
	...
	Width = width;
	Height = height;
	this->do_argb_conversion = do_argb_conversion;
...
cv::Mat CCameraOutput::Capture()
{
	cv::Mat result;
	const void *buffer;
	int buffer_len;
	if (!this->do_argb_conversion) {
		printf("Camera error: argb conversion must be enabled. Set the 5th argument of StartCamera to true.\n");
	}
	if (BeginReadFrame(buffer, buffer_len)) {
		// Create a temporary Mat object (zero-copy)
		cv::Mat raw_img (Height, Width, CV_8UC4, (void *)buffer);
		cv::cvtColor(raw_img, result, cv::COLOR_RGBA2BGR);
		EndReadFrame();
	}
	return result;
}
...
int CCameraOutput::ReadFrame(void* dest, int dest_size)
{
	...
	if(BeginReadFrame(buffer,buffer_len))
	{
		...
		else
		{
			...
			printf("CCameraOutput::ReadFrame: buffer too small. Need %i bytes, actual size %i bytes\n", buffer_len, dest_size);
			res = -1;
		}
		EndReadFrame();
	} else {
		printf("CCameraOutput::ReadFrame: no data available\n");
	}
